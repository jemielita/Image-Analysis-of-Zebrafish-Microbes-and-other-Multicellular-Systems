...
%Create handles to MIP and segmented outline.
imSize = size(imread(outFileList{frame2fileMap(1)}));
hIm = imshow(zeros(imSize));
hSeg = imshow(zeros(imSize));
...
for iFrame = 1 : numFramesMovie-1
    ...
    if frame2fileMap(iFrame) ~= 0 %if frame exists
        ...
        %Load in the segmented region
        imS = load(['E:\toConvert\Neutrophil_April20\fish1', filesep,...
            'seg_', num2str(iFrame+1), '.mat'], 'segMax');
        imS = imS.segMax;
        imS = bwperim(imS);
        se = strel('disk', 3);
        imS = imdilate(imS, se);
    ...
    %Crop segmented region down
    imS = imS(imageRange(1,1):imageRange(1,2),...
        imageRange(2,1):imageRange(2,2),:);
    ...
    %Plot the outline of the objects segmented in this frame
    imS = imS*intensityMinMax(2);
    hSeg = imshow(imS, intensityMinMax);
    set(hSeg, 'AlphaData', imS);
